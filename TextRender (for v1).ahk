...
class TextRender {
   ...
   InvalidateWindow() {
      this.Delete("WindowTime")
      this.Delete("WindowLeft")
      this.Delete("WindowTop")
      this.Delete("WindowRight")
      this.Delete("WindowBottom")
      this.Delete("WindowWidth")
      this.Delete("WindowHeight")
   }
   ...
   UpdateLayeredWindow(alpha := 255) {
      ...
      if (ExStyle & 0x8)                              ; WS_EX_TOPMOST
         WinSet AlwaysOnTop, On, % "ahk_id" this.hwnd ; Set always on top again.
   }
   ...
   FreeBitmap() {
      ...
      this.Delete("BitmapTop")
      this.Delete("BitmapRight")
      this.Delete("BitmapBottom")
   ...
   DrawBitmap(data := "", style1 := "", style2 := "") {
      ...
      that := this.DrawOnGraphics(this.Graphics
         ...
         , this.ViewportWidth
         , this.ViewportHeight
         , this.ViewportLeft + (this.HasKey("OriginLeft") ? this.WindowLeft - this.OriginLeft : 0)
         , this.ViewportTop + (this.HasKey("OriginTop") ? this.WindowTop - this.OriginTop : 0))
   ...
   DrawOnGraphics(Graphics, text := "", style1 := "", style2 := "", ViewportWidth := "", ViewportHeight := "", ViewportLeft := "", ViewportTop := "") {
      ...
      try dpi := DllCall("SetThreadDpiAwarenessContext", "ptr", -3, "ptr")
      SysGet MonitorGetCount, MonitorCount ; Use the coordinates of the screen index.
      if (_s ~= "^\d+$" && _s > 0 && _s <= MonitorGetCount) {
         SysGet Viewport, Monitor, % _s
   ...
   GetParentCoordinates(ByRef x, ByRef y, ByRef w, ByRef h) {
      try dpi := DllCall("SetThreadDpiAwarenessContext", "ptr", -3, "ptr")
...
class ImageRender extends TextRender {
   ...
   DrawOnGraphics(Graphics, image := "", style := "", polygons := "", ViewportWidth := "", ViewportHeight := "") {
      ...
      ; Default x and y.
      if (x == "")
         x := 0.5*ViewportWidth, a := 2+((( a-1)//3)*3)
      if (y == "")
         y := 0.5*ViewportHeight, a := 4+(mod( a-1,3))
